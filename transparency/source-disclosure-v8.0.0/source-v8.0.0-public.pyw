...
# Wersja 8.0.0: darmowa bez limitu czasu, Fix dla yt-dlp i tryby Mini do testów
...
try:
    ...
    messagebox.showerror(
        ...
        "Najprościej uruchom raz plik Uruchom_GUI_v8-0-0.bat albo zainstaluj:\n"
        "pip install customtkinter\n\n"
        "Jeśli stary folder zależności istnieje, ale program nadal pokazuje ten komunikat, "
        "uruchom Uruchom_GUI_v8-0-0.bat - plik utworzy czysty folder .runtime_deps_v8_0_0.",
    )
    root.destroy()
    raise SystemExit(1)
...
TRIAL_STATE_FILE_NAME = "trial_state_v8_0_0.json"
TRIAL_REGISTRY_KEY = r"Software\VideoAndSoundDownloaderPro"
TRIAL_REGISTRY_VALUE_NAME = "trial_state_v8_0_0"
...
MESSAGE_EXACT_EN = {
    ...
    "Ta wersja programu wymaga biblioteki CustomTkinter.\n\nNajprościej uruchom raz plik Uruchom_GUI_v8-0-0.bat albo zainstaluj:\npip install customtkinter\n\nJeśli stary folder zależności istnieje, ale program nadal pokazuje ten komunikat, uruchom Uruchom_GUI_v8-0-0.bat - plik utworzy czysty folder .runtime_deps_v8_0_0.": "This version of the program requires the CustomTkinter library.\n\nThe easiest way is to run Uruchom_GUI_v8-0-0.bat once or install:\npip install customtkinter\n\nIf the old dependency folder exists but the program still shows this message, run Uruchom_GUI_v8-0-0.bat - it will create a clean .runtime_deps_v8_0_0 folder.",
    "Gotowe.": "Ready.",
    ...
    "Fix: przy kolejnym buildzie ta wersja zostanie scalona z EXE przez Kompiluj_EXE_v8-0-0.bat.": "Fix: during the next build this version will be bundled into the EXE by Kompiluj_EXE_v8-0-0.bat.",
...
class VideoDownloaderApp(ctk.CTk):
    ...
    def media_item_from_info(self, info, fallback_url):
        ...
        return {
            ...
            "title": str(info.get("title") or info.get("fulltitle") or "Wykryty materiał").strip(),
    ...
    def append_document_media(self, document, document_url, results, extractor_label):
        ...
        for index, source in enumerate(dict.fromkeys(sources), start=1):
            if self.is_direct_media_url(source):
                media_name = Path(urlparse(source).path).name or parser.page_title or f"Materiał {index}"
                results.append({
                    "url": source,
    ...
    def start_web_discovery(self, link, folder, preset, missing, embed_subtitles):
        try:
            ...
            if error:
                ...
                self.after(0, lambda: self.show_info_dialog(error["title"], error["message"]))
                return
    ...
    def show_media_selection_dialog(self, items, folder, preset, embed_subtitles):
        dialog = ctk.CTkToplevel(self)
        dialog.title(f"Wykryte materiały ({len(items)})")
        dialog.geometry("780x500")
        dialog.transient(self)
        dialog.grab_set()
        ctk.CTkLabel(dialog, text="Wybierz jeden, kilka lub wszystkie materiały (dwuklik pobiera wybraną pozycję)", font=("Segoe UI", 16, "bold")).pack(padx=18, pady=(18, 2), anchor="w")
        ctk.CTkLabel(
            dialog,
            text="Aby zaznaczyć kilka pozycji: przytrzymaj lewy Ctrl i klikaj lewym przyciskiem myszy.",
            font=("Segoe UI", 11),
            text_color="#b9c7d8",
        ...
        media_list.heading("material", text="Materiał")
        media_list.heading("url", text="Bezpośredni adres")
        ...
        for index, item in enumerate(items, start=1):
            ...
            live_label = " [AKTYWNA TRANSMISJA — NIEDOSTĘPNA]" if item.get("is_live") else ""
        ...
        ctk.CTkButton(buttons, text="Zaznacz wszystkie", command=select_all).pack(side="left")
        ctk.CTkButton(buttons, text="Kopiuj link", command=copy_links).pack(side="left", padx=8)
        ctk.CTkButton(buttons, text="Pobierz zaznaczone", command=begin_download).pack(side="right")
        ...
        try:
            ...
            for index, item in enumerate(items, start=1):
                ...
                if code == 0:
                    ...
                    failures.append((item["title"], self.classify_download_error()))
    ...
    def build_arte_format_selector(self, arte_prefix, video_quality):
        ...
        if video_quality == "Mini":
            return (
                f"worstvideo{prefix_filter}[vcodec!=none][ext=mp4]+worstaudio{prefix_filter}[acodec!=none]/"
                f"worst{prefix_filter}[vcodec!=none][acodec!=none]/"
                "worstvideo[vcodec!=none]+worstaudio[acodec!=none]/"
                "worst[vcodec!=none][acodec!=none]"
            )

        return (
            f"bestvideo{prefix_filter}{height_limit}[vcodec!=none][ext=mp4]+bestaudio{prefix_filter}[acodec!=none]/"
            f"best{prefix_filter}{height_limit}[vcodec!=none][acodec!=none][ext=mp4]/"
            f"bestvideo{prefix_filter}{height_limit}[vcodec!=none]+bestaudio{prefix_filter}[acodec!=none]/"
            f"best{prefix_filter}{height_limit}[vcodec!=none][acodec!=none]"
        )
